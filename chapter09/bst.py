    ...
    def remove(self, key: Any) -> bool:
        ...
        while True:
            ...
            if key == p.key:        # key와 노드p의 키가 같으면
                ...
                if key < p.key:     # key쪽이 작으면
                    ...
                    p = p.eft               # 왼쪽 서브트리에서 검색
                else:
                    is_left_child = False   # 여기서 내려가는 것은 오른쪽자식
                    p = p.right             # 오른쪽 서브트리에서 검색
        ...
        if p.left is None:                  # p에 왼쪽 자식이 없으면
            ...
            if p is self.root:
                ...
                parent.right = p.left       # 부모의 왼쪽 포인터가 왼쪽 자식을 가리킴
        else:
            paernt = p
            left = p.left                   # 서브 트리안에서 가장 큰 노드
            is_left_child = True
