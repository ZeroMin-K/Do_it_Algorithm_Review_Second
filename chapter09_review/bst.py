    ...
    def remove(self, key: Any) -> bool:
        ...
        while True:
            ...
            if key == p.key:            # key와 노드 p의 키가 같으면
                ...
                if key < p.key:             # key 쪽이 작으면
                    is_left_child = True    # 여기서  내려가는 것은 왼쪽자식
                    p = p.left              # 왼쪽 서브트리에서 검색
                else:                       # key 쪽이 크면
        ...
        if p.left is None:                  # p에 왼쪽 자식이 없으면
            if p is self.root:
                ...
                parent.right = p.right      # 부모의 오른쪽 포인터가 오른쪽 자식 가르킴. 
        elif p.right is None:               # p에 오른쪽 자식이 없으면
            if p is self.root:
                self.root = p.left
            elif is_left_child:
                parent.left = p.left        # 부모의 오른쪽 포인터가 왼쪽 자식을 가리킴
            else:
                parent.right = p.left       # 부모의 오른쪽 포인터가 왼쪽 자식을 가르킴. 
        else:
            parent = p
            left = p.left                   # 서브트리 안에서 가장 큰노드
            is_left_child = True
            ...
            if is_left_child:
                parent.left = left      # left 삭제
            else:
                parent.right = left.left        # left삭제
        return True
