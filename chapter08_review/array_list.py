...
class ArrayLinkedList:
    ...
    def get_insert_index(self):
        ...
        if self.deleted == Null:                # 삭제 레코드는 존재하지 않음
            ...
            rec = self.deleted
            self.deleted = self.n[rec].dnext    # 프리 리스트에서 맨앞 rec꺼내기
            return rec
        ...
        if self.head != Null:
            if self.n[self.head].next == Null:      # 노드가 1개뿐이면
                ...
                while self.n[ptr].next != Null:
                    pre = ptr
                    ptr = self.n[ptr].next
                self.n[pre].next = Null             # pre는 삭제한 뒤의 꼬리노드
    ...
    def clear(self) -> None:
        # 모든 노드 삭제
        while self.head != Null:        # 리스트 전체가 빌대까지
...
class ArrayLinkedListIterator:
    # 클래스 ArrayLinekdList의 이터레이터용 클래스
