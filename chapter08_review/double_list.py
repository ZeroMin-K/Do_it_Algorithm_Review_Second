...
class DoubleLinkedList:
    ...
    def add_first(self, data: Any) -> None:
        # 맨 앞에 노드 삽입
        self.current = self.head        # 더미 노드 head의 바로뒤에 삽입
        self.add(data)
    ...
    def remove_first(self) -> None:
        # 머리 노드삭제
        self.current = self.head.next   # 머리노드 head.next 삭제
        self.remove_current_node()

    def remove_last(self) -> None:
        # 꼬리 노드삭제
        self.current = self.head.prev       # 꼬리노드 head.prev 삭제
        self.remove_current_node()

    def clear(self) -> None:
        # 모든 노드 삭제
        while not self.is_empty():          # 리스트 전체 빌때까지
